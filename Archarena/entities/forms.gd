...
class form:
	...
	func facing(which):
		# gets the face data of the face opposite to the given one.
		return faces[sides[oppo[sides.find(which)]]]
...
func _ready():
	...
	while filename != "":
		if shapes.has(filename.trim_suffix(".dat")) or shapes.empty():
